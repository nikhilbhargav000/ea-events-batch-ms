...
import com.easyapper.eventsbatchms.model.eventshigh.EventsHighUpcomingOccurrenceDto;
...
@Component
public class EventsHighProcessor implements ItemProcessor<EventsHighDto, List<EventDto>> {
	...
	@Override
	public List<EventDto> process(EventsHighDto readEvent) throws Exception {
		...
		//Other
		processorHelper.addMultiEvents_Cats(readEvent.getCats(), eventList);
		return eventList;
	}
	
	private void  addMultiEvents_DateAndTime(Pair<List<String>, List<String>> pair, EventsHighDto readEvent, 
			List<EventDto> eventList ) 
					throws DateFormatNotSupportedException, CloneNotSupportedException {
		List<EventsHighUpcomingOccurrenceDto> upcomingOccList = readEvent.getUpcoming_occurrences();
		...
		for(EventDto eventDto : eventList) {
			...
			for(int indexTime = 0 ; indexTime < startTimeList.size() ; indexTime++) {
				...
				for(EventsHighUpcomingOccurrenceDto upcomingOcc : upcomingOccList) {
				...
				newEventDto.setEvent_start_time(util.getEATimeFormatStr(startTime));
				newEventDto.setEvent_end_time(util.getEATimeFormatStr(endTime));
	...
	private Pair<List<String>, List<String>> getTimeArrPair(EventsHighDto readEvent) {
		List<EventsHighUpcomingOccurrenceDto> upcomingOccList = readEvent.getUpcoming_occurrences();
...
//	 */
//	private void addMultiEvents_Cats(EventsHighDto readEvent, 
...
//	}
	...
	private String getProcessedPrice(EventsHighDto readEvent){
		String processPrice = "";
		List<EventsHighPriceDto> priceList = readEvent.getPrice();
		double minPrice = priceList.get(0).getValue();
		double maxPrice = priceList.get(0).getValue();
		for(EventsHighPriceDto priceDto : priceList) {
