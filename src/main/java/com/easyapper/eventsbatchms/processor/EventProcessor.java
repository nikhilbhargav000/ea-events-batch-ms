...
import com.easyapper.eventsbatchms.model.readevent.OrglEventUpcomingOccurrenceDto;
...
@Component
public class EventProcessor implements ItemProcessor<OrglEventDto, List<EventDto>> {
	...
	@Override
	public List<EventDto> process(OrglEventDto readEvent) throws Exception {
	 ...
	 */
	private void addMultiEvents_UpcomingOccurences(OrglEventDto readEvent, 
			List<EventDto> eventList) throws CloneNotSupportedException{

		List<OrglEventUpcomingOccurrenceDto> upcomingOccList = readEvent.getUpcoming_occurrences();
		...
		for(EventDto eventDto : eventList) {
			...
			for(OrglEventUpcomingOccurrenceDto upcomingOcc : upcomingOccList) {
	 ...
	 * @throws CloneNotSupportedException
	 */
	private void addMultiEvents_Cats(OrglEventDto readEvent, 
			List<EventDto> eventList) throws CloneNotSupportedException{
	 ...
	 * @throws CloneNotSupportedException
	 */
	private void addMultiEvents_Price(OrglEventDto readEvent, 
			List<EventDto> eventList) throws CloneNotSupportedException{
		
		List<OrglEventPriceDto> priceList = readEvent.getPrice();
		List<EventDto> toRemoveEventList = new ArrayList<>();
		List<EventDto> toAddEventList = new ArrayList();
		for(EventDto eventDto : eventList) {
			toRemoveEventList.add(eventDto);
			String newPrice;
			for(OrglEventPriceDto priceDto : priceList) {
				EventDto newEventDto = (EventDto) eventDto.clone();
				toAddEventList.add(newEventDto);
